...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
    ...
    <div class="lg:col-span-2" x-data="{ 
        tab: 'guests', 
        search: '{{ $search }}',
        getFilteredCount(isRSVP = false) {
            let rows = Array.from(document.querySelectorAll(isRSVP ? '#rsvp-table-body tr' : '#guest-table-body tr'));
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            @if($total_guests > 0 || $search)
                <!-- Search Bar -->
                <div class="px-8 py-6 border-b border-slate-50">
                    <form action="{{ route('dashboard.invitations.guests', $invitation->id) }}" method="GET" class="relative group">
                        <div class="absolute inset-y-0 left-5 flex items-center pointer-events-none">
                            <svg class="w-5 h-5 text-slate-300 group-focus-within:text-rose-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        </div>
                        <input type="text" name="search" x-model="search" placeholder="Cari nama tamu..." 
                        class="w-full bg-slate-50 border border-slate-100 rounded-2xl pl-12 pr-5 py-4 text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none transition-all"
                        autocomplete="off">
                    <template x-if="search">
                        <button type="button" @click="search = ''; $nextTick(() => $el.closest('form').submit())" class="absolute inset-y-0 right-5 flex items-center text-slate-300 hover:text-rose-500 transition-colors">
                ...
                <div class="flex border-b border-slate-50">
                    ...
                    👥 <span x-text="search ? 'Hasil' : 'Semua'">{{ $search ? 'Hasil' : 'Semua' }}</span> (<span x-text="getFilteredCount(false)">{{ $guests->count() }}</span>)
                </button>
                <button @click="tab = 'rsvps'"
                    ...
                    class="flex-1 py-6 font-bold text-sm uppercase tracking-widest transition-all">
                    💌 RSVP (<span x-text="getFilteredCount(true)">{{ $guests->where('is_rsvp', true)->count() }}</span>)
                </button>
                </div>
            @endif
                ...
                @if($guests->isEmpty())
                    <div class="p-20 text-center">
                        ...
                        <p class="text-slate-400">
                            {{ $search ? 'Tidak ada tamu dengan nama "' . $search . '"' : 'Belum ada tamu yang ditambahkan.' }}
                        </p>
                    </div>
                @else
                    <!-- Alpine No Results Message -->
                    <div x-show="getFilteredCount(false) === 0" class="p-20 text-center" style="display: none;">
                        <div class="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl">
                            🔍
                        </div>
                        <p class="text-slate-400">Tidak ada tamu yang cocok dengan "<span x-text="search"></span>"</p>
                    </div>

                    <div class="overflow-x-auto" x-show="getFilteredCount(false) > 0">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody id="guest-table-body" class="divide-y divide-slate-50">
                                @foreach($guests as $guest)
                                    <tr class="hover:bg-slate-50/50 transition-colors" 
                                        x-show="search === '' || '{{ strtolower($guest->name) }}'.includes(search.toLowerCase())"
                                        ...
                                        <td class="px-8 py-6">
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                @if($guest->phone)
                                                   <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $guest->phone) }}?text={{ rawurlencode("Assalamu'alaikum warahmatullahi wabarakatuh \n\nBismillah, dalam kesadaran bahwa setiap langkah hidup adalah bagian dari takdir yang Allah rangkai dengan penuh hikmah...\n\nKami ingin menyampaikan kabar bahwa kami akan melangsungkan pernikahan:\n*" . $invitation->bride_name . " & " . $invitation->groom_name . "*\n\nSebagai bentuk hormat dan kemudahan, kami juga menyertakan informasi lengkap melalui tautan berikut:\n\n" . $guestLink . "\n\nDoa baik dari teman-teman semua sangat berarti bagi kami, semoga langkah ini dimudahkan dan diberkahi hingga akhir.\n\nTerima kasih atas perhatian dan doanya \nWassalamu'alaikum warahmatullahi wabarakatuh\n\n*" . $invitation->bride_name . " & " . $invitation->groom_name . "*") }}" target="_blank" class="p-2 bg-emerald-50 text-emerald-500 rounded-lg hover:bg-emerald-500 hover:text-white transition-all shadow-sm" title="Kirim WhatsApp">
            ...
            <div x-show="tab === 'rsvps'" style="display: none;">
                @if($guests->where('is_rsvp', true)->isEmpty())
                    <div class="p-20 text-center">
                        <div class="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl">
                            💌
                        </div>
                        <p class="text-slate-400">
                            {{ $search ? 'Tidak ada konfirmasi dengan nama "' . $search . '"' : 'Belum ada konfirmasi kehadiran (RSVP).' }}
                        </p>
                    </div>
                @else
                    <!-- Alpine No Results Message for RSVP -->
                    <div x-show="getFilteredCount(true) === 0" class="p-20 text-center" style="display: none;">
                        <div class="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl">
                            🔍
                        </div>
                        <p class="text-slate-400">Tidak ada RSVP yang cocok dengan "<span x-text="search"></span>"</p>
                    </div>

                    <div class="overflow-x-auto" x-show="getFilteredCount(true) > 0">
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50/50 hidden md:table-row">
                            ...
                            <tbody id="rsvp-table-body" class="divide-y divide-slate-50">
                                @foreach($guests->where('is_rsvp', true) as $rsvp)
                                    <tr class="hover:bg-slate-50/50 transition-colors"
                                        x-show="search === '' || '{{ strtolower($rsvp->name) }}'.includes(search.toLowerCase())"
                                        data-name="{{ $rsvp->name }}">
                                        <td class="px-8 py-6">
