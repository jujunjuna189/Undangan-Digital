    ...
    <main id="main-content" class="hidden relative z-20 bg-[#B6A68F] min-h-screen overflow-x-hidden">
        ...
        <section id="hadiah" class="py-24 px-4 relative bg-transparent text-center overflow-hidden">
            <div class="container mx-auto max-w-4xl relative z-10">
                ...
                <div class="grid md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                    ...
                    <div class="bg-black/5 backdrop-blur-sm rounded-[2rem] p-8 border border-black/5 shadow-sm" data-aos="fade-up">
                        <h3 class="font-serif text-lg font-bold mb-4 uppercase tracking-widest text-[#1e1e1e]">{{ $invitation->bank_name ?? 'BANK' }}</h3>
                        <p class="text-2xl font-bold mb-2 text-[#1e1e1e]">{{ $invitation->bank_account }}</p>
                        <p class="text-xs font-serif italic mb-6 text-[#1e1e1e]/60">Atas Nama: {{ $invitation->bank_holder }}</p>
                    ...
                    <div class="bg-black/5 backdrop-blur-sm rounded-[2rem] p-8 border border-black/5 shadow-sm" data-aos="fade-up" data-aos-delay="200">
                        <h3 class="font-serif text-lg font-bold mb-4 uppercase tracking-widest text-[#1e1e1e]">{{ $invitation->bank_name_2 ?? 'BANK' }}</h3>
                        <p class="text-2xl font-bold mb-2 text-[#1e1e1e]">{{ $invitation->bank_account_2 }}</p>
                        <p class="text-xs font-serif italic mb-6 text-[#1e1e1e]/60">Atas Nama: {{ $invitation->bank_holder_2 }}</p>
            ...
            <div class="container mx-auto max-w-5xl relative z-10">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-10">
                    <div class="space-y-3" data-aos="fade-up">
                        <div class="w-16 h-16 mx-auto rounded-full bg-black/5 flex items-center justify-center border border-black/10">
        ...
        <section id="rsvp" class="py-24 px-4 relative bg-transparent overflow-hidden">
            <div class="container mx-auto max-w-xl relative z-10 text-center">
                <h2 class="font-serif text-[#1e1e1e] text-4xl md:text-6xl font-bold mb-10 tracking-tighter" data-aos="fade-up">Daftar Kehadiran</h2>
                
                <div class="bg-black/5 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 border border-black/5 shadow-sm" data-aos="fade-up">
                    <form id="rsvpForm" action="{{ route('rsvp.store') }}" method="POST" class="space-y-6">
                        ...
                        <div class="text-left">
                            <label class="block text-xs font-serif italic text-black/60 mb-2 px-2">Konfirmasi Kehadiran</label>
                ...
                <div class="max-h-[600px] overflow-y-auto custom-scrollbar px-2">
                    <div id="rsvpList" class="space-y-6 pb-12">
                        @forelse($invitation->guests()->where('is_rsvp', true)->orderBy('updated_at', 'desc')->get() as $rsvp)
                            <div class="bg-black/5 backdrop-blur-sm rounded-[2rem] p-6 text-left border border-black/5 relative overflow-hidden transition-all duration-500 hover:shadow-md" data-aos="fade-up">
                                <div class="absolute left-0 top-0 h-full w-1.5 {{ $rsvp->is_attending ? 'bg-zinc-800' : 'bg-zinc-400' }}"></div>
                                <div class="flex justify-between items-start mb-2 pl-4">
                                    <div>
                                        <h4 class="font-bold text-[#1e1e1e]">{{ $rsvp->name }}</h4>
                                        <p class="text-[10px] text-black/40 uppercase tracking-widest font-serif">{{ $rsvp->updated_at->diffForHumans() }}</p>
                                    </div>
                                    <span class="text-[9px] px-2 py-0.5 rounded-full uppercase tracking-widest font-bold border border-black/20 {{ $rsvp->is_attending ? 'bg-black text-white' : 'bg-transparent text-black/40' }}">
                                        {{ $rsvp->is_attending ? 'Hadir' : 'Absen' }}
                                    </span>
                                </div>
                                <p class="text-sm italic text-[#1e1e1e]/80 pl-4 font-serif leading-relaxed">"{{ $rsvp->message }}"</p>
                            </div>
                        @empty
                            <div class="py-12 bg-black/5 rounded-[2rem] border border-dashed border-black/20" data-aos="fade-up">
